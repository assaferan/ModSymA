...
this intrinsic returns the modular symbols space
 Sum_(d|GG^d subset G)alpha_d(M)
If G is a subgroup of GG, then
this intrinsic returns the modular symbols space
Sum_(d|G^d subset GG)beta_d(M).
In this latter case, if 
Kernel(DirichletCharacter(M)) does not contain  G,
then the 0 space is returned.
}
   ...
   if IsMultiChar(M) then
      ...
      return MC_ModularSymbols_of_LevelG(M,G);
   end if;
   ...
   if not exists(pos) { pos : pos in [1..#M`other_levels] 
                            | M`other_levels[pos][1] eq G} then
      /****************************************************************
         M does not know the old modular symbols at level G.  Here's what 
         we do:
            (1) If this object has a creator, ask it for
                the modular symbols at level G. If so, done;
                otherwise continue with the next step.
            (2) If not, construct the space of symbols at level G.
       ****************************************************************/

      // Step 1.
      if assigned M`creator then
         Append(~M`other_levels, <G,ModularSymbols(M`creator,G)>);
      else
         // Step 2: We are the creator.  
         eps   := DirichletCharacter(M);
         F     := BaseField(M);
	    ...
	    quo, pi := Gamma0(1)/Gamma0(1);
	       ...
	       epsG := ChangeRing(Restrict(eps,G),BaseRing(M));
            ...
            if IsAmbientSpace(M) then
               ...
               Append(~M`other_levels, <G,ModularSymbols(AmbientSpace(M),G)!!M>);
            end if;
         end if;
         (M`other_levels[#M`other_levels][2])`creator := M;  
      end if;
