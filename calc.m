...
/****-*-magma-* EXPORT DATE: 2004-03-08 ************************************
   ...
   Added
   intrinsic HeckeAlgebraFields(M::ModSymA) -> SeqEnum
   ...
   *** empty log message ***
   ...
   Added intrinsic TwistedWindingElement(M::ModSymA, i::RngIntElt, eps::GrpDrchAElt) -> ModSymAElt
   ...
   *** empty log message ***
   ...
   Added Added
   Added intrinsic HeckeAlgebraFields(M::ModSymA) -> SeqEnum
   Added
   ...
   Added Added intrinsic TwistedWindingElement(M::ModSymA, i::RngIntElt, eps::GrpDrchElt) -> ModSymAElt
   Added
...
intrinsic HeckeAlgebraZBasis(M::ModSymA) -> SeqEnum
...
/*intrinsic HeckeAlgebraTensorQ(M::ModSymA) -> AlgMat
...
intrinsic HeckeAlgebra(M::ModSymA: Bound := -1) -> AlgMat
...
intrinsic DiscriminantOfHeckeAlgebra(M::ModSymA: 
                       Bound := -1) -> RngIntElt
...
/*intrinsic XXXTorsionBound(A::ModSymA, maxp::RngIntElt) -> RngIntElt
...
intrinsic CharpolyOfFrobenius(M::ModSymA, p::RngIntElt) -> RngUPolElt
...
intrinsic TorsionBound(M::ModSymA, n::RngIntElt) -> RngIntElt
...
intrinsic SubgroupOfTorus(M::ModSymA, x::ModSymAElt) -> RngIntElt
...
intrinsic SubgroupOfTorus(M::ModSymA, s::[ModSymAElt]) -> GrpAb, Map, Map
...
intrinsic CuspidalSubgroup(M::ModSymA) -> GrpAb, Map
...
intrinsic RationalCuspidalSubgroup(M::ModSymA) -> GrpAb
...
intrinsic ModularKernel(M::ModSymA) -> GrpAb
...
intrinsic ModularDegree(M::ModSymA) -> RngIntElt
...
intrinsic pNewModularDegree(M::ModSymA, p::RngIntElt) -> RngIntElt
...
intrinsic CongruenceModulus(M::ModSymA : 
                          Bound := -1) -> RngIntElt
...
intrinsic CongruenceGroup(M::ModSymA :
                          Bound := -1) -> GrpAb
...
intrinsic IntersectionGroup(M1::ModSymA, M2::ModSymA) -> GrpAb
...
intrinsic HeckeEigenvalueRing(M::ModSymA : Bound := -1) -> Rng, Map
...
intrinsic HeckeEigenvalueField(M::ModSymA) -> Fld, Map
...
intrinsic TwistedWindingElement(M::ModSymA, i::RngIntElt, eps::GrpDrchElt) -> ModSymAElt
...
intrinsic WindingElement(M::ModSymA) -> ModSymAElt
{The winding element \{0,oo\}.}
   return WindingElement(M,1);
end intrinsic;


intrinsic WindingElement(M::ModSymA, i::RngIntElt) -> ModSymAElt
...
intrinsic WindingElementProjection(M::ModSymA) -> ModTupFld
{The image under RationalMapping(M) of \{0,oo\}.}
   return WindingElementProjection(M,1);
end intrinsic;


intrinsic WindingElementProjection(M::ModSymA, i::RngIntElt) -> ModTupFld
...
intrinsic WindingLattice(M::ModSymA, j::RngIntElt : 
               Bound := -1)  -> Lat
...
/*intrinsic XXX_WindingSubmodule(M::ModSymA, j::RngIntElt 
                          : Bound := -1)  -> ModTupFld
...
intrinsic WindingSubmodule(M::ModSymA 
                          : Bound := -1)  -> ModTupFld
...
intrinsic WindingSubmodule(M::ModSymA, j::RngIntElt 
                          : Bound := -1)  -> ModTupFld
{"} // "
   require j ge 1 and j le Weight(M)-1 : 
     "Argument 2 must lie between 1 and k-1, inclusive, where k is the weight of argument 1.";
...
intrinsic
TwistedWindingSubmodule(M::ModSymA, j::RngIntElt, eps::GrpDrchAElt) -> ModTupFld
{The Hecke submodule of the vector space Phi(M) generated by the image of the
 jth eps-twisted modular winding element, where Phi is RationalMapping(M).
 Some care is needed when using a modular symbol space in a +1 or -1 quotient.}

   require 1 le j and j le (Weight(M)-1) : 
    "Argument 2 must be a critical integer, i.e., an integer between "*
    "1 and k-1, inclusive, where k is the weight of argument 1.";

   require BaseRing(eps) cmpeq BaseField(M):
    "The base fields of arguments 1 and 3 must be equal.";
...
intrinsic LRatio(A::ModSymA, j::RngIntElt 
                  : Bound := -1) -> FldRatElt
...
intrinsic LRatioOddPart(M::ModSymA, j::RngIntElt : Bound := -1) -> FldRatElt
...
/*
intrinsic pMaximalDiscriminantOfHeckeAlgebra(M::ModSymA, p::RngIntElt) 
                -> RngIntElt, SeqEnum
...
intrinsic pIndexInNormalizationOfHeckeAlgebra(M::ModSymA, p::RngIntElt) 
                   -> RngIntElt, RngIntElt
