...
intrinsic Representation(x::ModSymAElt) -> ModTupFldElt
...
intrinsic Representation(M::ModSymA) -> ModTupFld, Map, Map
...
{"} // "
   ...
   if not assigned M`sub_representation or 
      ...
      if not assigned M`al_decomp
             and Dimension(M) gt Degree(M) / 2 then // sometimes clever trick!?
...
intrinsic DualRepresentation(M::ModSymA) -> ModTupFld, Map, Map
{Same as DualVectorSpace}
   return DualVectorSpace(M);
end intrinsic;

intrinsic DualVectorSpace(M::ModSymA) -> ModTupFld
...
that is isomorphic to M as module of the Hecke algebra}
 ...
 //	       (not IsOfGammaType(M) and Level(M) mod p eq 0)  then
            ...
            // the optimized HeckeOperator on subspaces *uses*
            // ProjectionMatrix
...
intrinsic Lattice(M::ModSymA) -> Lat, Map
...
intrinsic IntegralRepresentation(M::ModSymA) -> Lat
{A basis over the integers for the integral modular symbols in 
...
intrinsic DualLattice(M::ModSymA) -> Lat, Map
