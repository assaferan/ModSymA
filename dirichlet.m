...
/****-*-magma-* EXPORT DATE: 2004-03-08 ************************************
   ...
   2009-04   (Steve)
             ...
             Made changes so that in a GrpDrchA G, when an element of G is created 
   ...
   2004-10-05: Fixed intrinsic 'eq' (G::GrpDrchA,H::GrpDrchA) -> BoolElt
   ...
   *** empty log message ***
   ...
   Added intrinsic MinimalBaseRingCharacter(eps::GrpDrchAElt) -> GrpDrchAElt
   ...
   Added
     GaloisConjugacyRepresentatives(S::[GrpDrchAElt]) -> SeqEnum
   and
     AbelianGroup(G::GrpDrchA) -> GrpAb, Map
   -- William
   ...
   Added
      ...
      intrinsic DistinguishedRoot(G::GrpDrchA) -> RngElt, RngIntElt
...
declare type GrpDrchA [GrpDrchAElt];

declare attributes GrpDrchA:  // Call this GrpDrchA object G.
...
declare type GrpDrchAElt;

declare attributes GrpDrchAElt: 
   Parent,
...
intrinsic ValueList(eps::GrpDrchAElt) -> SeqEnum
...
intrinsic DirichletGroup(m::RngIntElt) -> GrpDrchA
...
intrinsic DirichletGroup(m::RngIntElt,R::Rng) -> GrpDrchA
...
intrinsic DirichletGroup(m::RngIntElt,R::Rng,z::RngElt,r::RngIntElt) -> GrpDrchA
...
intrinsic DirichletGroupCopy(G::GrpDrchA) -> GrpDrchA
{Clone G}
...
intrinsic DirichletGroupFull(N::RngIntElt) -> GrpDrchA
{Same as FullDirichletGroup}
   return FullDirichletGroup(N);
end intrinsic;

intrinsic FullDirichletGroup(N::RngIntElt) -> GrpDrchA
...
intrinsic AbelianGroup(G::GrpDrchA) -> GrpAb, Map
...
intrinsic Elements(G::GrpDrchA) -> SeqEnum
...
intrinsic Random(G::GrpDrchA) -> GrpDrchAElt
...
/* Elements of GrpDrchA are represented as elements of (Z/N)^*,
...
intrinsic IsCoercible(G::GrpDrchA,x::.) -> BoolElt, GrpDrchAElt
{}
   if Type(x) eq GrpDrchAElt then
      ...
      return true, initGrpDrchAElt(G,G`UGroup!0); 
      ...
      return true, initGrpDrchAElt(G,G`UGroup!x); 
   end if;
   return false, "Invalid coercion.";
end intrinsic;

intrinsic NumberOfNames(G::GrpDrchA) -> RngIntElt {}
 return Ngens(G); end intrinsic;

intrinsic AssignNames(~G::GrpDrchA, S::[MonStgElt])
{Assign names to the generators of G.}
   require #S eq Ngens(G) : "Argument 2 must have length", Ngens(G);
   G`Labels := S;
end intrinsic;

intrinsic Name(G::GrpDrchA,i::RngIntElt) -> GrpDrchAElt
   {The ith generator.}
...
intrinsic '.'(G::GrpDrchA,i::RngIntElt) -> GrpDrchAElt
   {The ith generator.}
   require i ge 0 : "Argument 2 must be nonnegative";
   require i le Ngens(G) :  "Argument 2 can be at most", Ngens(G);
...
intrinsic Print(X::GrpDrchA, level::MonStgElt)
   {}
...
intrinsic Print(x::GrpDrchAElt, level::MonStgElt)
   {}
   G := x`Parent;
   r := G`OrderOfRoot;
   S := Eltseq(x`Element);
   ...
   if Order(x) eq 2 then m:=IdentifyKroneckerCharacter(x);
    if Abs(m) eq Modulus(x) then  printf "Kronecker character %o",m;
    else printf "Kronecker character %o in modulus %o",m,Modulus(x); end if;
    return; end if;
   for i in [1..Ngens(G)] do
	 a := S[i];
         m := G`OrdersUGroup[i];
...
intrinsic 'in'(x::., X::GrpDrchA) -> BoolElt
...
intrinsic Parent(x::GrpDrchAElt) -> GrpDrchA
   {}
   return x`Parent;
end intrinsic;


intrinsic 'eq' (G::GrpDrchA,H::GrpDrchA) -> BoolElt
...
intrinsic 'eq' (x::GrpDrchAElt,y::GrpDrchAElt) -> BoolElt
...
intrinsic BaseRing(G::GrpDrchA) -> Rng
   {The base ring of G.}
   return G`BaseRing;
end intrinsic;


intrinsic BaseRing(x::GrpDrchAElt) -> Rng
   {The base ring, i.e. the codomain, of x.}
   return x`BaseRing;
end intrinsic;


intrinsic Domain(x::GrpDrchAElt) -> Rng
   {The integers.}
   return Integers();
end intrinsic;


intrinsic Codomain(x::GrpDrchAElt) -> Rng
{The coefficient ring in which x takes values.}
   return x`BaseRing;
end intrinsic;


intrinsic Exponent(G::GrpDrchA) -> RngIntElt
{The exponent of G.}
...
intrinsic Order(G::GrpDrchA) -> RngIntElt
{The order of G.}
   return &*G`OrdersGens;
end intrinsic;

intrinsic '#'(G::GrpDrchA) -> RngIntElt
{The order of G.}
   return Order(G);
end intrinsic;

intrinsic Order(x::GrpDrchAElt) -> RngIntElt
{The order of x.}
   return Order(x`Element);
end intrinsic;


intrinsic Generators(G::GrpDrchA) -> SeqEnum
{A sequence containing the generators for G}
   return [ G.i : i in [1..Ngens(G)] ];
end intrinsic;

intrinsic GeneratorsSequence(G::GrpDrchA) -> SeqEnum
{"} // "
   return [ G.i : i in [1..Ngens(G)] ];
end intrinsic;


intrinsic UnitGenerators(G::GrpDrchA) -> SeqEnum
...
intrinsic NumberOfGenerators(G::GrpDrchA) -> RngIntElt
   {The number of generators for G.}
   return #G`Labels;
end intrinsic;


intrinsic Modulus(G::GrpDrchA) -> RngIntElt
   {The modulus of the group G of Dirichlet characters.}
   return G`Modulus;
end intrinsic;

intrinsic Modulus(x::GrpDrchAElt) -> RngIntElt
...
intrinsic Conductor(x::GrpDrchAElt) -> RngIntElt
...
/*
intrinsic ConductorOld(x::GrpDrchAElt) -> RngIntElt
...
intrinsic IsIdentity(chi::GrpDrchAElt) -> BoolElt
{True iff chi is the trivial character}
   return IsIdentity(chi`Element);
end intrinsic;

intrinsic IsTrivial(chi::GrpDrchAElt) -> BoolElt
{"} // "
   return IsIdentity(chi`Element);
end intrinsic;

intrinsic IsEven(chi::GrpDrchAElt) -> BoolElt
{True iff the Dirichlet character chi satisfies chi(-1) = 1}
   return Evaluate(chi,-1) eq 1;
end intrinsic;

intrinsic IsOdd(chi::GrpDrchAElt) -> BoolElt
{True iff the Dirichlet character chi satisfies chi(-1) = -1}
   return Evaluate(chi,-1) eq -1;
end intrinsic;

intrinsic IsTotallyEven(chi::GrpDrchAElt) -> BoolElt
...
intrinsic IsPrimitive(x::GrpDrchAElt) -> BoolElt
  {True iff the conductor equals the modulus}
   return Modulus(x) eq Conductor(x);
end intrinsic;

intrinsic AssociatedPrimitiveCharacter(x::GrpDrchAElt) -> GrpDrchAElt
...
intrinsic Evaluate(x::GrpDrchAElt,n::RngIntElt) -> RngElt
...
intrinsic Evaluate(x::GrpDrchAElt,n::RngIntResElt) -> RngElt
{Evaluation x(n).}
   return Evaluate(x, Integers()!n);
end intrinsic;

intrinsic '@'(n::RngIntElt, x::GrpDrchAElt) -> RngElt
{"} // "
   return Evaluate(x,n);
end intrinsic;

intrinsic '@'(n::RngIntResElt, x::GrpDrchAElt) -> RngElt
{"} // "
   return Evaluate(x,n);
end intrinsic;

intrinsic '*' (x::GrpDrchAElt,y::GrpDrchAElt) -> GrpDrchAElt
...
equal then the base ring has to be cyclotomic.}
   ...
   if Gx eq Gy then
      if Parent(x`Element) cmpeq Parent(y`Element) then
         return initGrpDrchAElt(Gx,x`Element+y`Element);
      end if;
      xe := Eltseq(x);
      ye := Eltseq(y);
      assert #xe eq #ye;
...
intrinsic '^' (x::GrpDrchAElt,n::RngIntElt) -> GrpDrchAElt
   {The n-th power of the Dirichlet character x}
   if n eq 1 then 
      return x;
   end if; 
   return initGrpDrchAElt(x`Parent, n*x`Element);
end intrinsic;

intrinsic Extend(x::GrpDrchAElt, m::RngIntElt) -> GrpDrchAElt
...
intrinsic Extend(x::GrpDrchAElt, H::GrpDrchA) -> GrpDrchAElt
...
intrinsic Eltseq(x::GrpDrchAElt) -> SeqEnum
{For internal use}
   return Eltseq(x`Element);
end intrinsic;


intrinsic Decomposition(x::GrpDrchAElt) -> List
...
intrinsic Restrict(x::GrpDrchAElt,m::RngIntElt) -> GrpDrchAElt
...
intrinsic Restrict(x::GrpDrchAElt,H::GrpDrchA) -> GrpDrchAElt
...
intrinsic BaseExtend(G::GrpDrchA, R::Rng) -> GrpDrchA
...
intrinsic BaseExtend(G::GrpDrchA, R::Rng, z::RngElt) -> GrpDrchA
...
intrinsic BaseExtend(eps::GrpDrchAElt, R::Rng) -> GrpDrchAElt
...
intrinsic BaseExtend(eps::GrpDrchAElt, R::Rng, z::RngElt) -> GrpDrchAElt
{Base extension of eps to R.}
...
intrinsic BaseExtend(G::GrpDrchA, f::Map) -> GrpDrchA
...
intrinsic BaseExtend(eps::GrpDrchAElt, f::Map) -> GrpDrchAElt
...
intrinsic KroneckerCharacter(D::RngIntElt) -> GrpDrchAElt
...
intrinsic KroneckerCharacter(D::RngIntElt, R::Rng) -> GrpDrchAElt
   {"} // "
...
intrinsic GaloisConjugacyRepresentatives(G::GrpDrchA) -> SeqEnum
...
intrinsic GaloisConjugacyRepresentatives(S::[GrpDrchAElt]) -> SeqEnum
...
// Extending to arbitrary representations of finite groups

intrinsic GaloisConjugacyRepresentatives(S::[ModGrp]) -> SeqEnum
{Representatives for the Gal(Qbar/Q)-conjugacy classes of finite group
 representations contained in the given sequence S}

    grps := {Domain(Representation(s)) : s in S};
    require #grps eq 1 : "All representations must be of the same group";

    G := SetToSequence(grps)[1];
    ...
    for idx in [1..#S_parts] do
        ...
        Gal := Automorphisms(F);
        S_reps := [Representation(s) : s in S_parts[idx]];
...
intrinsic MinimalBaseRingCharacter(eps::GrpDrchAElt) -> GrpDrchAElt
...
intrinsic ValuesOnUnitGenerators(x::GrpDrchAElt) -> SeqEnum
...
intrinsic DirichletCharacterFromValuesOnUnitGenerators(G::GrpDrchA, v::SeqEnum) -> GrpDrchAElt
...
// and a DirichletCharacter on success or 0 on failure. 
   ...
   return true, "", initGrpDrchAElt(G, G`UGroup!elt);
end function;

intrinsic ApplyAutomorphism(x::GrpDrchAElt, phi::.) -> GrpDrchAElt
{Same as x^phi}
...
intrinsic '^'(x::GrpDrchAElt, phi::.) -> GrpDrchAElt
...
intrinsic Sqrt(eps::GrpDrchAElt) -> GrpDrchAElt
...
intrinsic GaussSum(eps : GrpDrchAElt) -> FldCycElt
