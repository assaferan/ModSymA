...
/****-*-magma-* EXPORT DATE: 2004-03-08 ************************************
   ...
   09/07/20: (Eran) Modified NewSubspace, prepare_old_spaces, 
             ...
             Added the function GetModSymAPrimes to be used both here and in decomp.m.
             Modified NewSubspace accordingly.

   08/16/03: (WAS) Fixed a bug in NewSubspace(M::ModSymA) where a circular
...
import "../GrpGL2Hat/misc.m" : IsConjugate,
       MinimalOvergroups,
       NormalizerGrpMat;
...
intrinsic ZeroSubspace(M::ModSymA) -> ModSymA
...
intrinsic CuspidalSubspace(M::ModSymA) -> ModSymA
...
intrinsic EisensteinSubspace(M::ModSymA) -> ModSymA
...
/*  ***READ THIS BEFORE YOU EVEN THINK OF IMPORTING ONE OF THE BELOW FUNCTIONS
   ...
   end user value and allow one to create ModSymA objects in bizarre ways
   ...
   with ModSymA's need not determine an ideal which cuts out the dual
   representation.  
...
intrinsic BasisPlus(M::ModSymA) -> SeqEnum
{Basis for the +1 eigenspace of StarInvolution on M.}
   return IntegralBasis(PlusSubspace(M));
end intrinsic;

intrinsic BasisMinus(M::ModSymA) -> SeqEnum
{Basis for the -1 eigenspace of StarInvolution on M.}
   return IntegralBasis(MinusSubspace(M));
end intrinsic;

intrinsic IntegralBasisPlus(M::ModSymA) -> SeqEnum
{Integral basis for the +1 eigenspace of StarInvolution on M.}
   return IntegralBasis(PlusSubspace(M));
end intrinsic;

intrinsic IntegralBasisMinus(M::ModSymA) -> SeqEnum
{Integral basis for the -1 eigenspace of StarInvolution on M.}
   return IntegralBasis(MinusSubspace(M));
end intrinsic;

intrinsic NewSubspace(M::ModSymA, p::GrpMat : ComputeDual := false) -> ModSymA
{The p-new subspace of M. This is the complement in M of the subspace
    generated by the modular symbols of level subgroup p and character
    the restriction of the character of M. If the character of M does not
    restrict, then NewSubspace(M,p) is equal to M. Note that M is required
    to be cuspidal.}
...
intrinsic NewSubspace(M::ModSymA, p::RngIntElt) -> ModSymA
...
over all prime divisors of the level of M}
   ...
   if not assigned M`new_part then
      ...
      if IsMultiChar(M) then
         ...
         // !!! TODO : adapt this one to arbitrary gamma as well
...
intrinsic Complement(M::ModSymA) -> ModSymA
...
intrinsic OldSubspace(M::ModSymA) -> ModSymA
...
intrinsic Projection(M::ModSymA) -> Map
...
intrinsic ProjectionMap(M::ModSymA) -> Map
{"} // "
   pi := ProjectionMatrix(M);
   return hom<AmbientSpace(M) -> AmbientSpace(M) | x :-> x*pi>;
end intrinsic;

intrinsic ProjectionMatrix(M::ModSymA) -> AlgMatElt
{"} // "
