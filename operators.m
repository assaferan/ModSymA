...
/****-*-magma-* EXPORT DATE: 2004-03-08 ************************************
   ...
   temporarily use
     intrinsic Hecke(M::ModSymA, Heil::SeqEnum) -> .
   ...
...
intrinsic HeckeOperator(M::ModSymA, n::RngIntElt) -> AlgMatElt
...
intrinsic IntegralHeckeOperator(M::ModSymA, n::RngIntElt) -> AlgMatElt
...
function HeckeGeneralCaseRepresentativesDoubleCoset2(G, alpha)
  ...
  if (GCD(det,N) eq 1) then
    ...
    import "../GrpGL2Hat/creation.m" : get_coercion_hom;
...
intrinsic HeckeAdelic(M::ModSymA, w::GrpMatElt[FldRat]) -> AlgMatElt
...
intrinsic HeckeAdelicComponents(M::ModSymA, w::GrpMatElt[FldRat]) -> AlgMatElt
...
intrinsic HeckeImagesSquarePrimes(M::ModSymA, i::RngIntElt,
				  n::RngIntElt) -> SeqEnum
{The images of the ith standard basis vector
...
intrinsic HeckeImagesAll(M::ModSymA, i::RngIntElt, n::RngIntElt) -> SeqEnum
{The images of the ith standard basis vector
...
intrinsic HeckeSpan(v::ModSymAElt
                     : Bound := -1 ) -> Lat
...
intrinsic HeckeFieldSpan(v::ModSymAElt : 
                         Bound := -1) -> Lat
...
intrinsic HeckeBound(M::ModSymA) -> RngIntElt
...
intrinsic SetHeckeBound(M::ModSymA, n::RngIntElt) 
...
intrinsic DualHeckeOperator(M::ModSymA, n::RngIntElt) -> AlgMatElt
...
intrinsic AtkinLehner(M::ModSymA, q::RngIntElt) -> AlgMatElt
{The same as AtkinLehnerOperator(M,q).}
   return AtkinLehnerOperator(M,q);   
end intrinsic;

intrinsic AtkinLehnerOperator(M::ModSymA, q::RngIntElt) -> AlgMatElt 
...
intrinsic AtkinLehnerOperatorOverQ(M::ModSymA, q::RngIntElt) -> AlgMatElt 
...
intrinsic DualAtkinLehner(M::ModSymA, q::RngIntElt) -> AlgMatElt
{The same as DualAtkinLehnerOperator(M,q).}
   return DualAtkinLehnerOperator(M,q);
end intrinsic;


intrinsic DualAtkinLehnerOperator(M::ModSymA, q::RngIntElt) -> AlgMatElt
...
intrinsic DualStarInvolution(M::ModSymA) -> AlgMatElt
...
intrinsic StarInvolution(M::ModSymA) -> AlgMatElt
...
intrinsic ThetaOperator(M1::ModSymA, M2::ModSymA) -> Map
...
the weight of M_1 plus p+1.}
   ...
   function do_ThetaOperator(M1, M2, x)
      assert Type(M1) eq ModSymA;
      assert Type(M2) eq ModSymA;
...
intrinsic HeckeOperatorModSymA(M::ModSymA, p::RngIntElt) -> AlgMatElt
